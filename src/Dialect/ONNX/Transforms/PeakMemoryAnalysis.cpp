...
struct PeakMemoryAnalysis
  ...
  StringRef getDescription() const override {
    return "Analyzes peak memory usage (Parameters as baseline, Activations as "
           "live).";
  }
  ...
  void runOnOperation() override {
    ...
    funcOp.walk([&](Operation *op) {
      ...
      logFile << "Total: " << totalBytes
              << " bytes | Activations: " << liveActivBytes << " bytes | Op: ";
      op->print(logFile);
      logFile << "\n---------------------------------\n";
    ...
    // 요약 출력
    llvm::outs() << "[PeakMemoryAnalysis] Peak TOTAL memory: " << peakTotal
                 << " bytes (Parameters: " << baselineBytes
    ...
    for (Value v : blk->currentlyLiveValues(peakOp))
      liveVals.insert(v);
...
private:
  ...
  static int64_t getTensorSize(TensorType tensorType) {
    ...
    for (int64_t d : tensorType.getShape()) {
      if (d == ShapedType::kDynamic) {
        // 동적 차원 발견 시 에러
        llvm::errs()
            << "[PeakMemoryAnalysis] Error: dynamic dimension found in type: "
            << tensorType << "\n";
        return -1;
      }
      total *= d;
  ...
  static int64_t calculateActivationUsageAtOp(
      Operation *op, Liveness &liveness) {
